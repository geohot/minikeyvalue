...
if [[ -z "${USERPASS}" ]]; then
  ...
  # rebuild and compare the database
  ./mkv -userpass $USERPASS -volumes localhost:3001,localhost:3002,localhost:3003,localhost:3004,localhost:3005 -db $ALTDB rebuild
  go run tools/leveldb_compare.go /tmp/indexdb/ $ALTDB

  # do a rebalance, then put it back
  ./mkv -userpass $USERPASS -volumes localhost:3001,localhost:3002,localhost:3003 -db $ALTDB rebalance
  ./mkv -userpass $USERPASS -volumes localhost:3001,localhost:3002,localhost:3003,localhost:3004,localhost:3005 -db $ALTDB rebalance
  go run tools/leveldb_compare.go /tmp/indexdb/ $ALTDB

  # rebuild and compare the database
  ./mkv -userpass $USERPASS -volumes localhost:3001,localhost:3002,localhost:3003,localhost:3004,localhost:3005 -db $ALTDB2 rebuild
  go run tools/leveldb_compare.go /tmp/indexdb/ $ALTDB2
fi
